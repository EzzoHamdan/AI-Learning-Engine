...
from learning_engine.google_ai_client import ChatCompletions, GoogleAIClient
from learning_engine.local_ai_client import (
    create_local_client,
    is_ollama_running,
    list_available_models,
)
from learning_engine.session_manager import SessionManager
from learning_engine.settings import GoogleAIConfig, LocalAIConfig
...
class AIClientFactory:
    ...
    def _create_local_client(self) -> tuple[Any, str, bool]:
        ...
        try:
            ...
            model_to_use = getattr(
                st.session_state, "selected_local_model", self.local_ai_config.MODEL_NAME
            )
            ...
            client = create_local_client(base_url=ollama_base_url, model=model_to_use)
            ...
            return self._create_fallback_client(f"Local AI error: {str(e)}")
    ...
    def _create_google_client(self) -> tuple[Any, str, bool]:
        ...
        try:
            ...
            if not api_key:
                return self._create_fallback_client("Google AI API key not provided")
            ...
            return self._create_fallback_client(f"Google AI error: {str(e)}")
    ...
    def _create_openai_client(self) -> tuple[Any, str, bool]:
        ...
        try:
            ...
            if not api_key:
                return self._create_fallback_client("OpenAI API key not provided")
            ...
            return self._create_fallback_client(f"OpenAI error: {str(e)}")
