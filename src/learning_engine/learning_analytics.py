...
class LearningAnalytics:
    ...
    def track_flashcard_interaction(self, action: str):
        ...
        if action == "viewed":
            ...
            st.session_state.engagement_metrics["flashcard_interactions"][
                "incorrect_responses"
            ] += 1
        elif action == "skipped":
    ...
    def add_to_learning_history(self, activity_type: str, data: dict):
        ...
        entry = {"timestamp": datetime.now(), "type": activity_type, "data": data}
    ...
    def display_session_overview(self):
        ...
        with col2:
            ...
            st.metric(
                "Quizzes Completed", total_quizzes, help="Number of quizzes completed this session"
            )

        with col3:
            total_materials = st.session_state.materials_analytics["total_materials"]
            st.metric(
                "Materials Generated", total_materials, help="Study materials created this session"
            )
    ...
    def display_materials_analytics(self):
        ...
        with col3:
            ...
            if generation_times:
                avg_time = sum(
                    gt["generation_time"] for gt in generation_times if gt["success"]
                ) / len([gt for gt in generation_times if gt["success"]])
        ...
        if generation_times:
            success_rate = (
                sum(1 for gt in generation_times if gt["success"]) / len(generation_times) * 100
            )
    ...
    def display_progress_tracking(self):
        ...
        if quiz_analytics["performance_over_time"]:
            scores = [
                entry["score_percentage"] for entry in quiz_analytics["performance_over_time"]
            ]
        ...
        with col1:
            ...
            # Score progress bar
            score_bar_color = (
                "green" if score_progress >= 100 else "orange" if score_progress >= 70 else "red"
            )
