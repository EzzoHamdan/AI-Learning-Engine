...
class LocalAIClient:
    ...
    def generate_completion(
        self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, stream: bool = False
    ) -> CompletionResponse:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return CompletionResponse(choices=[choice], model=self.model)
            else:
                raise Exception(
                    f"Local AI request failed: {response.status_code} - {response.text}"
                )
...
def create_local_client(
    base_url: str = "http://127.0.0.1:11434", model: str = "gemma2:2b"
) -> MockLocalOpenAIClient:
    """
